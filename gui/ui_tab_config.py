...
class ConfigUI(AbstractComponentUI):
    ...
    def save_keys(self, openai_key, eleven_key, pexels_key, huggingface_key):
        ...
        if (self.api_key_manager.get_api_key('HUGGINGFACE') != huggingface_key):
            self.api_key_manager.set_api_key("HUGGINGFACE", huggingface_key)
        return gr.Textbox.update(value=openai_key),\
            gr.Textbox.update(value=eleven_key),\
            gr.Textbox.update(value=pexels_key),\
            gr.Textbox.update(value=huggingface_key),\
                ...
                with gr.Column():
                    ...
                    with gr.Row():
                        with gr.Column(scale=40):
                            openai_textbox = gr.Textbox(value=self.api_key_manager.get_api_key("OPENAI"), label=f"OPENAI API KEY (Optional)", show_label=True, interactive=True, show_copy_button=True, type="password", scale=40)
                    ...
                    with gr.Row():
                        with gr.Column(scale=40):
                            huggingface_textbox = gr.Textbox(value=self.api_key_manager.get_api_key("HUGGINGFACE"), label=f"HUGGINGFACE API KEY (Optional)", show_label=True, interactive=True, show_copy_button=True, type="password", scale=40)
                            show_huggingface_key = gr.Button("Show", size="sm", scale=1)
                            show_huggingface_key.click(self.on_show, [show_huggingface_key], [huggingface_textbox, show_huggingface_key])
                    with gr.Row():
                        with gr.Column(scale=40):
                    ...
                    save_button.click(self.verify_eleven_key, [eleven_labs_textbox, eleven_characters_remaining], [eleven_characters_remaining]).success(
                        self.save_keys, [openai_textbox, eleven_labs_textbox, pexels_textbox, huggingface_textbox], [openai_textbox, eleven_labs_textbox, pexels_textbox, huggingface_textbox, AssetComponentsUtils.voiceChoice(), AssetComponentsUtils.voiceChoiceTranslation()])
