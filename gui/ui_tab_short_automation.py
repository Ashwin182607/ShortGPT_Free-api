...
class ShortAutomationUI(AbstractComponentUI):
    ...
    def create_ui(self):
        with gr.Row(visible=False) as short_automation:
            with gr.Column():
                ...
                tts_engine = gr.Radio([AssetComponentsUtils.ELEVEN_TTS, AssetComponentsUtils.EDGE_TTS, AssetComponentsUtils.COQUI_TTS, "Google TTS"], label="Text to speech engine", value=AssetComponentsUtils.EDGE_TTS, interactive=True)
                ...
                with gr.Column(visible=False) as gtts:
                    language_gtts = gr.Dropdown([lang.value.upper() for lang in Language], label="Language", value="ENGLISH", interactive=True)
    ...
    def create_short(self, numShorts, short_type, tts_engine, language_eleven, language_edge, language_coqui, language_gtts, numImages, watermark, background_video_list, background_music_list, facts_subject, voice_eleven, voice_coqui, progress=gr.Progress()):
        '''Creates a short'''
        try:
            ...
            if tts_engine == AssetComponentsUtils.ELEVEN_TTS:
                ...
                language = Language(language_gtts.lower().capitalize())
                voice_module = GTTSVoiceModule(language)
    ...
    def create_ui(self):
        with gr.Row(visible=False) as short_automation:
            with gr.Column():
                ...
                tts_engine.change(tts_engine_change, tts_engine, [eleven_tts, edge_tts, coqui_tts, gtts])
