...
class ContentShortEngine(AbstractContentEngine):
    ...
    def _generateScript(self):
        if self.short_type.endswith("_short"):
            # Extract topic from short_type (e.g., "reddit_relationships_short_1" -> "reddit relationships")
            topic = " ".join(self.short_type.split("_")[:-2])
            script_data = gpt_batch_video.generate_batch_script(topic, self._db_language)
            self._db_script = script_data['script']
            self._db_title = script_data['title']
        else:
            self._db_script = ""
